...
This updated README provides a complete **zero-to-running** guide including:

- **Exact system prerequisites** with Homebrew install commands
- **PostgreSQL database & user creation** with SQL commands
- **Step-by-step dependency installation** with a manual fallback table
- **Environment variable generation** (including `openssl` for NEXTAUTH_SECRET)
- **Prisma 7.x specific workflow** (`prisma.config.ts`, driver adapter, generate → migrate → seed)
- **Seed data credentials** so the user can log in immediately
- **Verification steps** to confirm everything works
- **Full affiliation policy documentation** with examples table
- **Troubleshooting section** for common Mac/PostgreSQL issues
This updated README provides a complete **zero-to-running** guide including:
