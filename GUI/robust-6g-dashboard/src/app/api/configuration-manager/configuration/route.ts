...
export async function GET(request: NextRequest) {
  const configId = request.nextUrl.searchParams.get("config_id")?.trim();

  if (!configId) {
    return NextResponse.json(
      { status: "error", message: "Missing config_id query parameter." },
      { status: 400 },
    );
  }
  ...
  const backendUrl = new URL(
    `${backendBaseUrl}/ConfigurationManager/getConfiguration`,
  );
  backendUrl.searchParams.set("config_id", configId);
